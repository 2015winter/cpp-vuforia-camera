...
bool AppDelegate::applicationDidFinishLaunching(cv::Mat* gray, cv::Mat* mat, pthread_mutex_t* mutex) {
    ...
    if(!glview) {
        ...
        glview = GLViewImpl::createWithRect("cpp-vuforia-camera", Rect(0, 0, designResolutionSize.width, designResolutionSize.height));
#else
        glview = GLViewImpl::create("cpp-vuforia-camera");
#endif
